...
export function login(apiUrl) {
    ...
    // Login header
    let login_h = document.createElement("h1");
    let login_text = document.createTextNode("Login");
    login_h.appendChild(login_text);
    div.appendChild(login_h);
    ...
    let submit = document.createTextNode("Login");
        ...
        if (username.localeCompare("") == 0) {
            ...
            .then((res) => {
                if (res.status == 200) {
                    ...
                    printmsg("Error: Missing username/password");
                }
                else if (res.status == 403) {
                    printmsg("Error: Incorrect username/password");
                }
            });
        }
    });
